...
$showPayableFormHeader = $showPayableFormHeader ?? true;
$showPayableFormNavigation = $showPayableFormNavigation ?? true;
$showPayableStatementModal = $showPayableStatementModal ?? true;
...
<div x-data="payableForm()" data-orders="<?= esc($ordersJson, 'attr') ?>" class="space-y-6">
    <?php if ($showPayableFormHeader): ?>
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <?php if ($showPayableFormNavigation): ?>
    ...
    <form class="space-y-6" method="post" action="<?= base_url('payables') ?>">
            ...
            <div class="flex min-w-[1040px] gap-6">
                <div class="card w-[440px] shrink-0 p-4">
                    <div class="flex items-center justify-between gap-3">
                        <h2 class="text-sm font-semibold">Unpaid RRs</h2>
                        <span class="text-xs muted" x-text="filteredOrders().length + ' items'"></span>
                    </div>
                    <table class="table mt-4">
                        <thead><tr><th>RR#</th><th>Date</th><th>Due Date</th><th>Balance</th><th class="text-right">Pay</th></tr></thead>
                        <tbody>
                            <template x-if="pagedOrders().length === 0"><tr><td colspan="5">No unpaid RRs.</td></tr></template>
                            <template x-for="order in pagedOrders()" :key="order.purchase_order_id"><tr><td x-text="order.po_no"></td><td x-text="order.date"></td><td x-text="order.due_date || '-'"></td><td x-text="formatAmount(order.working_balance)"></td><td class="text-right"><button class="btn btn-secondary btn-strong" type="button" @click="openPayModal(order)" :disabled="Number(order.working_balance) <= 0">Pay</button></td></tr></template>
                        </tbody>
                    </table>
                    ...
                    <div class="mt-4 flex flex-wrap items-center justify-between gap-3 text-sm">
                        <span class="muted" x-text="paginationLabel()"></span>
                        <div class="flex gap-2">
                            <button class="btn btn-secondary" type="button" @click="previousPage()" :disabled="currentPage <= 1">Prev</button>
                            <button class="btn btn-secondary" type="button" @click="nextPage()" :disabled="currentPage >= totalPages()">Next</button>
                        </div>
                    </div>
                </div>
        ...
        <div class="card p-4">
            ...
            <div class="mt-4 flex flex-wrap items-end gap-3">
                <button class="btn btn-strong" type="submit" <?= $activeReceipt ? '' : 'disabled' ?>>Commit Payment</button>
                <?php if ($cancelAction): ?>
                    <button class="btn btn-secondary" type="button" @click="<?= esc($cancelAction, 'attr') ?>"><?= esc($cancelButtonLabel) ?></button>
                <?php else: ?>
                    <a class="btn btn-secondary" href="<?= base_url('payables/supplier/' . $supplier['id']) ?>"><?= esc($cancelButtonLabel) ?></a>
                <?php endif; ?>
            </div>
        </div>
    </form>
    ...
    <?php if ($showPayableStatementModal): ?>
...
<script>
    function payableForm() {
        return {
            ...
            filteredOrders() { return this.visibleOrders; },
            totalPages() { return Math.max(1, Math.ceil(this.filteredOrders().length / Number(this.pageSize || 1))); },
            pagedOrders() {
                if (this.currentPage > this.totalPages()) {
                    this.currentPage = this.totalPages();
                }

                const start = (this.currentPage - 1) * Number(this.pageSize || 1);
                return this.filteredOrders().slice(start, start + Number(this.pageSize || 1));
            },
            paginationLabel() {
                const total = this.filteredOrders().length;
                if (total === 0) return 'No RRs to show';

                const start = ((this.currentPage - 1) * Number(this.pageSize || 1)) + 1;
                const end = Math.min(start + Number(this.pageSize || 1) - 1, total);
                return 'Showing ' + start + '-' + end + ' of ' + total;
            },
            previousPage() { if (this.currentPage > 1) this.currentPage -= 1; },
