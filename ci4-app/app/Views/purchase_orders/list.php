...
$listUrl = $supplier ? base_url('suppliers/' . $supplier['id'] . '/purchase-orders') : base_url('purchase-orders');
$printParams = ['from_date' => $fromDate ?? '', 'to_date' => $toDate ?? '', 'po_no' => $poNo ?? ''];
if ($supplier) {
    $printParams['supplier_id'] = $supplier['id'];
}
?>
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
            ...
            <?php if ($supplier): ?>
                ...
                <a class="btn btn-secondary" href="<?= base_url('payables/supplier/' . $supplier['id']) ?>">Payables</a>
            <?php endif; ?>
    ...
    <table class="table mt-6">
        ...
        <tbody>
            <?php if (empty($purchaseOrders)): ?>
                <tr><td class="py-3" colspan="<?= $supplier ? '9' : '10' ?>">No purchase orders found for the selected filters.</td></tr>
            <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td class="text-right">
                            <button
                                ...
                                <?= ((float) $order['balance'] > 0 && $quickPayActiveReceipt) ? '' : 'disabled' ?>>
                                Pay
                            </button>
                        </td>
        ...
        <div class="modal-panel max-h-[92vh] max-w-4xl overflow-y-auto p-6" @click.stop>
            ...
            <form method="post" action="<?= base_url('payables/quick-pay') ?>" class="space-y-5" x-on:submit.prevent="if (Math.abs(quickPayBalanceAmount()) > 0.005) { window.showToast('Unallocated amount must be zero before committing.', 'error'); return; } $el.submit();">
                ...
                <div class="grid gap-5 lg:grid-cols-3">
                    ...
                    <div class="card p-4"><h3 class="text-sm font-semibold">A/P Other</h3><div class="mt-4 grid gap-4"><div><label class="block text-sm font-medium">Description</label><input class="input mt-1" name="ar_other_description" x-model="quickPay.arOtherDescription"></div><div><label class="block text-sm font-medium">Amount</label><input class="input mt-1" name="ar_other_amount" type="number" step="0.01" min="0" x-model="quickPay.arOtherAmount"></div></div></div>
                </div>
                <div class="card p-4">
                    <div class="grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-5">
                        ...
                        <div><p class="muted">A/P Other</p><p class="font-semibold" x-text="formatAmount(quickPay.arOtherAmount)"></p></div>
    ...
    function purchaseOrderList() {
        return {
            ...
            quickPayBalanceAmount() {
                return (parseFloat(this.quickPay.amountReceived) || 0) + this.quickPayFixedAccountsTotal() - (parseFloat(this.quickPay.allocationAmount) || 0) - (parseFloat(this.quickPay.arOtherAmount) || 0);
            },
